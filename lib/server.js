...
Server.CONFIG = {
  SERVER: {
    ...
    OUTPUT_TEX: 'server.output_tex'
  },
  SLACK: {
    TOKEN: 'slack.token',
    COMMAND: 'slack.command',
    WEBHOOK_URL: 'slack.webhook_url',
    LINK_TEXT: 'slack.link_text'
  },
  TEX: {
    BIN: 'tex.bin',
...
Server.prototype.handle = function(request, reply)
{
  ...
  var context = new Context(
    ...
    this.config.get(Server.CONFIG.SLACK.LINK_TEXT)
    );
  ...
  var outputTex = this.config.get(Server.CONFIG.SERVER.OUTPUT_TEX);
  ...
  writer.write(context.texFile)
    ...
    .then(function(file)
    {
      ...
      if(outputTex == 'true'){
        fs.copy(context.texFile, context.outFile + ".tex", function(err){
          if(err) Console.log("Could not copy tex file");
        });
      }
